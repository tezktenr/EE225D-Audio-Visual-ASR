...
from src.audio_visual_asr.model.VideoModel import LipReading
from src.audio_visual_asr.dataset.LRW.LRW_VideoDataset import LRW_VideoDataset
...
def get_model_save_path(config):
    base_save_path = FileUtil.joinPath(CURR_SOURCE_DIR_PATH, "_SAVED_MODELS", "VideoOnly")
...
def getVideoConfig():
    ...
    parser = argparse.ArgumentParser(description='Train Video Only ASR Model')
    ...
    config = ConfigUtil.readVideoTrainConfig(args.config)
    return config
...
def trainVideoOnly():
    ...
    if use_gpu:
        ...
        raise RuntimeError("GPU cuda training is still an unimplemented or incomplete feature in 'trainVideoOnly.py'")
